...
def _password_matches(user: User, password: str, *, email_login: bool) -> bool:
    if email_login:
        stored = user.email_password_hash
...
def user_login():
    try:
        ...
        if not _password_matches(user, password, email_login=is_email_login):
            return jsonify({'success':False,'message':'密码错误！'}),401

        user.last_login_at = datetime.datetime.now(timezone.utc)
        db.session.commit()
        token = _issue_jwt_token({
            'user_id': user.id,
